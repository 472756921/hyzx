<template>
  <div>
      ...
      <Col span="8"  v-for="(item, i) in order" key="i">
        <div class="order">
          <div>
            <div>
              <span class="orderTitle">
                <span>{{ item.orderClass==1?'现金单':'匿名现金单' }}</span>
          ...
          <div>
            ...
            <span class="orderLiCon" v-for="(it,i) in item.p_name">
              {{ it.label }} &nbsp;<span class="price">￥{{ it.price }}</span> &nbsp;&nbsp;
              <span class="">分配现金业绩：{{ it.fpyj }}</span>&nbsp;&nbsp;
              <span class="">分配比例：{{ it.fpbl }}</span>&nbsp;&nbsp;
              <br/>
            </span>
          </div>
          <div>
            <div class="orderLititle">产品名称：</div>
            <span class="orderLiCon" v-for="(it,i) in item.pr_list">
              {{it.label}} <span class="orderNumber">X {{ it.number }}</span> &nbsp;
              <span class="price">￥{{ it.price }}</span>&nbsp;&nbsp;
              <span>折扣：{{ it.zk }}</span>&nbsp;&nbsp;
              实价：<span class="price">￥{{ it.zk*it.price/10 }}</span>&nbsp;&nbsp;
              <br/>
            </span>
          </div>
          <div>其他现金</div>
          <div>现金疗程：{{item.lc.name}} - {{ item.lc.type }} - <span class="price">￥{{ item.lc.price }}</span></div>
          <div>现金充值：{{item.cz.name}} - <span class="price">￥{{ item.cz.price }}</span> </div>
          <div>会员升级：{{item.sj.name}} - <span class="price">￥{{ item.sj.price }}</span></div>
          <div>现金购买活动卡：{{item.gk.name}} - {{ item.gk.type }} - <span class="price">￥{{ item.gk.price }}</span></div>
    ...
    <Modal  v-model="service" :title="serCard" @on-ok="ok"  >
      <Checkbox v-model="single"  :disabled="serCard=='修改现金单'?true:false">匿名现金单</Checkbox>
      <br/>
      <br/>
      <span>用户选择：</span>
      ...
      <br/>
      <span v-if="serCard!='修改现金单'">产品选择：</span>
      <span v-if="serCard=='修改现金单'">增加产品：</span>
